...
| parent_task_id | uuid | Hayir | Alt gorev icin referans |
| title | string(160) | Evet | Gorev basligi |
| description | text | Hayir | Detay |
| status | enum(draft, planned, in_progress, blocked, on_hold, completed, cancelled) | Evet | Yasam dongusu |
| priority | enum(low, normal, high, critical) | Evet | Oncelik |
| owner_id | uuid | Evet | Sorumlu kisi |
| reporter_id | uuid | Hayir | Gorevi acan kisi |
| planned_start / planned_end | datetime | Hayir | Takvim |
| actual_start / actual_end | datetime | Hayir | Gercek zaman |
| progress_pct | int | Evet | %0-100 |
| effort_planned_hours | decimal | Hayir | Tahmini emek |
| effort_logged_hours | decimal | Hayir | Gerceklesen |
| risk_level | enum(none, low, medium, high) | Hayir | Risk etiketi |
| tags | string[] | Hayir | Serbest etiket |
| linkage | jsonb | Hayir | Ilgili gorev/dokuman/kpi baglari |
...
| doc_type | enum(contract, report, plan, requirement, risk, generic) + custom | Evet | Sistem tipi veya custom |
...
Dokuman turleri icin sistem varsayilanlari kullanilir, ancak adminler custom listeye yeni tur ekleyebilir. Etiket (tag) alanlari serbest metin, fakat adminler sozluk onerebilir.
...
- **Notification**: Kullaniciya gonderilen bildirimler (email, webhook).
...
- FR-51: Opsiyonel Slack/Teams webhook bildirimi.
...
| Rapor | GET | /api/v1/reports/audit | Audit log export | Denetci |
...
- **MVP**: SMTP (mail bildirim), S3 (dosya depolama), ClamAV, Opsiyonel Slack/Teams webhook.
